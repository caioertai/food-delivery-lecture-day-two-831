class Router
  ...
  def run
    while @running
      @current_user = @sessions_controller.login # creates a sessions / presence or not of a user
      while @current_user
  ...
  def print_rider_menu
    ...
    puts "------- MENU -------"
    puts "--------------------"
    puts "1. Mark one of my undelivered orders as delivered"
  ...
  def rider_action(choice)
    ...
    when 2 then @orders_controller.list_my_undelivered(@current_user)
    when 5 then logout!
    when 6 then stop!
    else puts "Try again..."
    end
